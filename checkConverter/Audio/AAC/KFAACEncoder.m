...
- (void) setupAACEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    outAudioStreamBasicDescription.mFormatFlags = 0; //kMPEG4Object_AAC_LC; // Format-specific flags to specify details of the format. Set to 0 to indicate no format flags. See “Audio Data Format Identifiers” for the flags that apply to each format.
    ...
    if (self.bitrate != 0) {
        ...
        OSStatus result = AudioConverterSetProperty(_audioConverter, kAudioConverterEncodeBitRate, ulSize, & ulBitRate);
        assert(result == noErr);
    }
}
...
- (size_t) copyPCMSamplesIntoBuffer:(AudioBufferList*)ioData {
    ...
    assert(_pcmBuffer!=NULL);
    assert(_pcmBufferSize > 0);
    _pcmBuffer = NULL;
    _pcmBufferSize = 0;
...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    NSLog(@"sampleBuffer %@", CMSampleBufferGetNumSamples(sampleBuffer));
